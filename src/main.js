...
function installMenu() {
  ...
  $.each(gb.menu.items, function(k, v) {
    var item = $('<li class="item">' + gb.menu[v].text + '</li>');
    menu.append(item);
    var ul = null;
    $.each(gb.menu[v].items, function(sk, sv) {
      ...
      if (sv == '-') {
        ...
        var sitem = $('<li class="sub-item">' + gb.menu[v][sv].text + '</li>');
        ul.append(sitem);
...
window.init = function() {
  ...
  gb.currentDoc = doc;
  doc.active();
  if (window.localStorage && localStorage[doc.title])
    doc.load(eval('(' + localStorage[doc.title] + ')'));
};
